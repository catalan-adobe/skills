...
import { runCluster, slugify, treeBootstrap, treeFromPoll } from './cluster.mjs';

const config = {
  origin: 'https://example.com',
  overlaySelectors: ['#cookie'],
  concurrency: { browser: 2 },
  thresholds: { clusterSimilarity: 0.8, minClusterSize: 2, representativesPerTemplate: 2 },
...
test('fingerprints candidates resumably and then finalizes templates', async () => {
  ...
  const first = await runCluster({
    config, paths, browserFactory: fakeBrowserFactory(log), limit: 2, shots: false,
  });
...
test('a reduce error is stored as fingerprintError and counted in the tally', async () => {
  ...
  const summary = await runCluster({
    config, paths, browserFactory: failing, limit: 1, shots: false,
  });
...
test('force with a limit keeps stale URLs remaining instead of finalizing', async () => {
  ...
  const forced = await runCluster({
    config, paths, browserFactory: fakeBrowserFactory(log), force: true, limit: 1, shots: false,
  });
...
test('representative screenshots are captured once and skipped when already on disk', async () => {
  const paths = await seed();
  const log = [];
  const first = await runCluster({ config, paths, browserFactory: fakeBrowserFactory(log) });
  ...
  await runCluster({ config, paths, browserFactory: fakeBrowserFactory(log) });
